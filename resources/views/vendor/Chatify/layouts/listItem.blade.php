...
@if ($get == 'saved')
    @php
        $user = Auth::user();
    @endphp
    <table class="messenger-list-item {{ $user->channel_id ? 'contact-item' : 'search-item' }}"
        data-channel="{{ $user->channel_id }}" data-user="{{ $user->id }}">
        <tr data-action="0">
            {{-- Avatar side --}}
            <td>
...
@if ($get == 'search_item')
    <table class="messenger-list-item search-item" data-user="{{ $user->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p>{{ strlen($user->full_name) > 12 ? trim(substr($user->full_name, 0, 12)) . '..' : $user->full_name }}</p>
            </td>
        </tr>
    </table>
@endif
