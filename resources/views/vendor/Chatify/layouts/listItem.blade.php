{{-- -------------------- Saved Messages -------------------- --}}
@if ($get == 'saved')
    <table class="messenger-list-item {{ Auth::user()->channel_id ? 'contact-item' : 'search-item' }}"
        data-channel="{{ Auth::user()->channel_id }}" data-user="{{ Auth::user()->id }}">
        <tr data-action="0">
            {{-- Avatar side --}}
            <td>
    ...
    <table class="messenger-list-item contact-item" data-channel="{{ $channel->id }}">
        <tr data-action="0">
            ...
            <td style="position: relative">
                ...
                @endif
                <div class="avatar av-m" style="background-image: url('{{ $user->avatar_url }}');">
                </div>
            </td>
            {{-- center side --}}
            <td>
...
@if ($get == 'search_item')
    <table class="messenger-list-item search-item" data-user="{{ $user->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p>{{ strlen($user->user_name) > 12 ? trim(substr($user->user_name, 0, 12)) . '..' : $user->user_name }}
                </p>
            </td>
        </tr>
    </table>
...
@if ($get == 'user_search_item')
    <table class="user-list-item" data-user="{{ $user->id }}">
        <tr data-action="0">
            {{-- Avatar side --}}
            <td>
                <div class="avatar av-s" style="background-image: url('{{ $user->avatar_url }}');">
                </div>
            </td>
            {{-- center side --}}
            <td>
